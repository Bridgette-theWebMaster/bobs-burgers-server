...
router.get('/orders', authorize, async (req, res) => {
    try {
        const allOrders = await pool.query("SELECT * FROM orders WHERE uid = $1", [req.allOrders])
...
router.post('/orders', async(req, res) => {
    ...
    const createOrder = await pool.query("INSERT INTO orders (user_id, order_date) VALUES ($1, $2) RETURNING *",
        [uid, order_date])
...
router.post('/items', async (req, res) => {
    try {
        ...
        const createItem = await pool.query("INSERT INTO items (bid, oid, quantity, price) VALUES ($1, $2, $3, $4) RETURNING *",
